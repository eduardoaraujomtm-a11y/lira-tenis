...
export function CategoriaManager({ categoryId }: { categoryId: string }) {
  ...
  const load = useCallback(async () => {
    const [catRes, athRes, compRes, courtRes, tourRes, matchRes] = await Promise.all([
      supabase.from("categories").select("id,name,short_name,type,format,rule").eq("id", categoryId).single(),
      ...
      supabase.from("tournaments").select("days").limit(1).single(),
      supabase
        .from("matches")
        .select("id,phase,group_id,round,time,status,sets,winner_id,competitor_a,competitor_b")
        .eq("category_id", categoryId),
    ]);
  ...
  const { bracketGroups, groupGames } = useMemo(() => {
    ...
    const toView = (m: RawMatch): MatchView => {
      const side = (id: string | null, pick: "a" | "b") => ({
        ...
        sets: (m.sets ?? []).map((s) => ({
          ...
          tb: s.tbA !== undefined && s.tbB !== undefined ? Math.min(s.tbA, s.tbB) : undefined,
        })),
    ...
    const bracketGroups = order
      .map((phase) => ({
        ...
        matches: knockout.filter((m) => m.phase === phase).map(toView),
      }))
  ...
  async function addDupla(e: React.FormEvent) {
    ...
    const { data: comp, error: e1 } = await supabase
      ...
      .single();
    if (e1 || !comp) {
      setError("Erro ao criar a dupla. Você está logado?");
      setBusy(false);
      return;
    }
  ...
  async function insertGenerated(gen: GenMatch[]) {
    ...
    const rows = sorted.map((g, i) => ({
      id: keyToId.get(g.key)!,
      category_id: categoryId,
      phase: g.phase,
      group_id: g.groupId ?? null,
      round: g.round ?? null,
      day,
      time: slotTime(i, courts.length),
  ...
  async function generateKO() {
    ...
    try {
      await clearMatches();
      const gen = generateKnockout(comps.map((c) => ({ id: c.id, seed: c.seed })));
  ...
  async function generateQualifierKO() {
    ...
    const qualifiers = qualifiersSeeded(standings, 2);
  ...
  return (
    <div>
      <Link href="/admin/chaveamento" className="mb-2 inline-block text-xs font-semibold text-lira-purple">
        ← Categorias
      </Link>
      ...
      <CategoriaSettings cat={cat} onSaved={load} />
            ...
            {comps.map((c) => (
              <li key={c.id} className="flex items-center justify-between gap-2 px-3 py-2 text-sm">
                <span className="flex items-center gap-2 truncate">
                  ...
                  {c.group_id && (
                    <span className="rounded bg-lira-purple-soft px-1.5 text-[11px] font-bold text-lira-purple">
                      G{c.group_id}
                    </span>
                  )}
      ...
      <div className="rounded-xl border border-border bg-card p-3">
        ...
        {showGroups && showKnockout && (
          <button
            ...
            className="rounded-lg border border-lira-purple px-4 py-2 text-sm font-bold text-lira-purple disabled:opacity-40"
          >
        ...
        <p className="mt-2 text-xs text-muted">
          {showGroups && showKnockout
            ? "Gere os grupos, jogue-os, e depois gere o mata-mata a partir da classificação (1º A × 2º B…)."
            : showGroups
            ? "Todos jogam contra todos dentro de cada grupo."
            : "As cabeças de chave ficam nas posições padrão e sobras viram bye."}
        </p>
      </div>
      ...
      {matchCount > 0 && (
        <div className="mt-6">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Link href="/chaves" className="text-xs font-semibold text-lira-purple">
              Ver no site →
            </Link>
          </div>
...
function CategoriaSettings({ cat, onSaved }: { cat: Category; onSaved: () => void }) {
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from("categories")
      .update({ name, short_name: shortName || name, type, format, rule })
      .eq("id", cat.id);
    setBusy(false);
  ...
  return (
    <div className="mb-4">
      <button
        onClick={() => setOpen((o) => !o)}
        className="text-xs font-semibold text-lira-purple"
      >
      ...
      {open && (
        <div className="mt-2 rounded-xl border border-border bg-card p-3">
          ...
          {msg && <p className="mb-2 text-xs font-semibold text-lira-purple">{msg}</p>}
